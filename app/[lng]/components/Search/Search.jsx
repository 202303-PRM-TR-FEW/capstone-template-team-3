  ...
  return (
    <form>
      ...
      {searchCampaigns.length > 0 && isDropdownOpen && (
        <div
          className={`absolute overflow-y-auto z-[10] bg-accent text-gray-900  mt-1 p-2.5 border-0 shadow-lg h-fit rounded-lg ${style}`}
          style={{
            top: window.innerWidth < 1280 ? "230%" : "100%",
            width: window.innerWidth < 1280 ? "12.4rem" : "20rem",
          }}
        >
          <ul className="suggestions-list">
            {suggestions.length > 0 ? (
              suggestions.map((campaign) => (
                ...
                >
                  <button>{campaign.data.projectName.slice(0, 14)}</button>
                  <div className="ml-2">
                    <img
                      src={campaign.data.image}
                      alt={campaign.data.projectName}
                      className="w-14 h-7"
                    />
                  </div>
                </div>
              ...
              <div className="no-results-message">
                <span className="text-sm text-gray-900/50">
                  {t("Campaigns not found")}
                </span>
              </div>
            )}
          </ul>
