...
const Campaigns = ({ params }) => {
  ...
  return userStatus === "loading" || campaignStatus === "loading" ? (
    ...
    <main>
      ...
      <div className="flex flex-col justify-center items-center bg-theme p-5 w-1/3 mx-auto rounded-xl gap-3 min-w-max my-5">
        <p>{t("Thanks to our supporters")}</p>
        <div className="bg-accent-black text-accent p-5 rounded-xl">
          <span>{`$${totalCharity}`}</span>
        </div>
        <h3>{t("accumulated for charity")}!</h3>
      </div>
      <CategoryFilter />
