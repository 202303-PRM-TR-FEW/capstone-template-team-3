...
const PaymentModal = ({ campaignId }) => {
    ...
    const onSubmit = async (data) => {
        ...
        await dispatch(addUserDonation({ currentUserId, donation, checkbox, campaignId }))
    ...
    return (
        <main>
            <div className="flex items-center justify-center fixed top-0 left-0 w-screen h-screen bg-zinc-950 bg-opacity-50 modal-background">
                <form onSubmit={handleSubmit(onSubmit)} className="bg-slate-50 lg:w-[35%] lg:h-[70%] rounded-xl p-4 flex flex-col justify-between sm:w-[75%] sm:h-[75%]">
                    <div>
                        <div>
                        ...
                        <div>
                            <input {...register("donation", { required: true, pattern: /^[1-9][0-9]*$/ })}
                                placeholder="$0"
                                className="bg-slate-50 text-black text-[30px] w-full border-none focus:outline-none focus:ring-0 custom-border-bottom" />
                            {errors.donation?.type === 'required' && <p role="alert" className="text-end text-red-600 italic text-[14px]">Donation is required</p>}
                            {errors.donation?.type === 'pattern' && <p role="alert" className="text-end text-red-600 italic text-[14px]">Donation is invalid</p>}
                        ...
                        <div className='mt-3 flex items-center'>
                            ...
                            <input {...register("checkbox")} type="checkbox" name="checkbox" className="ml-2 charity-checkbox text-theme" />
                        </div>
                    </div>
                    <div className='flex justify-end mt-10 md:mt-auto'>
                        <Button type="submit" style='bg-zinc-950 rounded-md w-full p-2 text-white text-[15px]'>
                            Pay now
                        </Button>
                    </div>
                </form>
